...
export default function BirdDetail({ bird: initialBird, username, region, prevId, nextId }: Props) {
  ...
  const api = `/api/users/${username}/birds/${bird.id}`;
  ...
  async function toggleDiscovered() {
    const res = await fetch(api, {
      ...
      body: JSON.stringify({ discovered: discovered ? 0 : 1 }),
    });
    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--bg)' }}>
      ...
      >
        <div className="flex items-center gap-4">
          <Link href={`/user/${username}`} className="text-white text-sm hover:text-red-200 flex items-center gap-1.5 transition-colors">
            <span>←</span><span>Back</span>
          </Link>
          <div className="w-px h-4 bg-red-800" />
          <span className="text-white font-bold text-lg tracking-wide">
            {capitalize(username)}&rsquo;s {region} BirdDex
          </span>
        </div>
        <div className="flex items-center gap-2">
          {prevId && (
            <Link href={`/user/${username}/bird/${prevId}`} className="text-red-200 hover:text-white text-sm px-3 py-1 rounded transition-colors" style={{ background: 'rgba(0,0,0,0.2)' }}>
              ← #{padId(prevId)}
            </Link>
          )}
          {nextId && (
            <Link href={`/user/${username}/bird/${nextId}`} className="text-red-200 hover:text-white text-sm px-3 py-1 rounded transition-colors" style={{ background: 'rgba(0,0,0,0.2)' }}>
              #{padId(nextId)} →
            </Link>
          )}
        </div>
      ...
      <div className="flex-1 max-w-5xl mx-auto w-full px-4 py-8 flex flex-col gap-8">
          ...
          <div className="flex-1 flex flex-col gap-3">
            ...
            >
              {activePhoto ? (
                ...
                <img src={activePhoto.url} alt={bird.name} className="max-h-80 max-w-full object-contain" draggable={false} />
              ) : (
              ...
              {photos.length > 1 && safeIndex > 0 && (
                <button onClick={goPrev} className="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center hover:scale-110 transition-all" style={{ background: 'rgba(255,255,255,0.85)', color: 'var(--text)', boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }}>
                  <ChevronLeft />
                </button>
              )}
              {photos.length > 1 && safeIndex < photos.length - 1 && (
                <button onClick={goNext} className="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center hover:scale-110 transition-all" style={{ background: 'rgba(255,255,255,0.85)', color: 'var(--text)', boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }}>
                  <ChevronRight />
                </button>
              )}
              {photos.length > 1 && (
                <div className="absolute bottom-2 right-3 text-xs font-mono px-2 py-0.5 rounded-full" style={{ background: 'rgba(0,0,0,0.35)', color: 'white' }}>
            ...
            {activePhoto && isOwner && (
              <div className="flex items-center gap-3 px-1">
                ...
                <button onClick={deleteActivePhoto} className="text-xs text-red-500 hover:text-red-700 transition-colors shrink-0">Delete photo</button>
              </div>
            )}
            {activePhoto && !isOwner && activePhoto.caption && (
              <p className="text-xs px-1 italic" style={{ color: 'var(--text-muted)' }}>"{activePhoto.caption}"</p>
            )}
                ...
                {photos.map((photo, i) => {
                  ...
                  return (
                    <div key={photo.id} className="flex flex-col items-center gap-1">
                      <button onClick={() => setActiveIndex(i)} className={`rounded-lg overflow-hidden w-14 h-14 border-2 transition-all shrink-0 ${isActive ? 'border-green-400 opacity-100' : 'border-[#b8d0e4] opacity-50 hover:opacity-80'}`}>
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={photo.url} alt={photo.caption || bird.name} className="w-full h-full object-cover" />
                      </button>
                      {isOwner && (
                        <button onClick={() => setCover(photo.id)} title={isCover ? 'Cover photo' : 'Set as cover'} className="text-[11px] transition-colors leading-none" style={{ color: isCover ? '#ca8a04' : 'var(--text-dim)' }}>
                          {isCover ? '★' : '☆'}
                        </button>
                      )}
                    </div>
          ...
          <div className="w-full md:w-72 flex flex-col gap-4">
            <div className="rounded-2xl p-5" style={{ background: 'var(--panel)', border: `1px solid ${discovered ? '#86efac' : 'var(--card-border)'}` }}>
              <div className="flex items-start justify-between mb-2">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full font-mono ${discovered ? 'bg-green-100 text-green-700' : 'text-[#6a90a8]'}`} style={!discovered ? { background: 'var(--card-border)' } : {}}>
            ...
            {isOwner && (
              <div className="rounded-2xl p-4 flex flex-col gap-3" style={{ background: 'var(--panel)', border: '1px solid var(--card-border)' }}>
                ...
                <input type="text" value={caption} onChange={e => setCaption(e.target.value)} placeholder="Caption (applied to all)" className="w-full px-3 py-2 rounded-lg text-sm outline-none" style={{ background: 'white', border: '1px solid var(--card-border)', color: 'var(--text)' }} />
        ...
        <div className="rounded-2xl p-6" style={{ background: 'var(--panel)', border: '1px solid var(--card-border)' }}>
          ...
          {isOwner ? (
            ...
            <p className="text-sm font-mono leading-relaxed whitespace-pre-wrap" style={{ color: bird.field_notes ? 'var(--text)' : 'var(--text-dim)' }}>
...
function Spinner() {
  return <svg className="animate-spin" width="14" height="14" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" strokeDasharray="60" strokeDashoffset="20" /></svg>;
}
